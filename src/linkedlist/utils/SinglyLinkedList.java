...
{
    public ListNode<T> head;                              //头指针，指向单链表的头结点

    public SinglyLinkedList()                         //默认构造方法，构造空单链表
    {
        this.head = new ListNode<T>();                    //创建头结点，val和next值均为null
    }
    ...
    {
        this();                                       //创建空单链表，只有头结点
        ListNode<T> rear=this.head;                       //rear指向单链表最后一个结点
        for (int i=0; i<element.length; i++)          //若element==null，抛出空对象异常
        {                                             //element.length==0时，构造空链表
            rear.next=new ListNode<T>(element[i],null);   //尾插入,创建结点链入rear结点之后
    ...
    {
        int i=0; 
        ListNode<T> p=this.head.next;                     //p从单链表第一个结点开始
    ...
    {
        String str="(";
        for (ListNode<T> p = this.head.next;  p!=null;  p=p.next) 
        {   str += p.val.toString();
            if (p.next!=null) 
                str += ",";                                //不是最后一个结点时后加分隔符
        }
...
/*    //插入x作为p结点的后继结点，若操作成功返回新插入结点；否则返回null，O(1)
    ...
    public void insert(int i, T x) 
    {
        ...
        ListNode<T> p=this.head;                          //p指向头结点
        for (int j=0;  p.next!=null && j<i;  j++)     //寻找插入位置
            p = p.next;                               //循环停止时，p指向第i-1结点或最后一个结点
        p.next = new ListNode<T>(x, p.next);              //插入x作为p结点的后继结点，包括头插入（i<=0）、中间/尾插入（i>0）
    }
...
//        throw new IndexOutOfBoundsException(i+"");         //抛出序号越界异常
    ...
    */
    ...
    public SinglyLinkedList(SinglyLinkedList<T> list)
    {
        ...
        for (ListNode<T> p=list.head.next;  p!=null;  p=p.next)  //若list==null，抛出空对象异常
        {
            rear.next = new ListNode<T>(p.val, null);
            rear = rear.next; 
        }
    }
...
/*    public boolean equals(Object obj)
    {
        ...
        while (p!=null && q!=null && p.val.equals(q.val))
        {
            p=p.next;
            q=q.next;
        }
    ...
    //(1)以下4个方法提供迭代遍历单链表方式
    public ListNode<T> getFirst()                    //返回单链表第一个元素结点（非头结点），O(1)
    {
        return this.head.next;                   //若单链表空返回null
    }
    public ListNode<T> getNext(ListNode<T> p)            //返回p的后继结点，O(1)
    {
        if (this.head.next==null || p==null)
            return null;
        return p.next;
    }
    public ListNode<T> getPred(ListNode<T> p)            //返回p的前驱结点，O(n)
    {
        if (p==null || this.head.next==p)
            return null;
        ListNode<T> front=this.head.next;
        while (front!=null && front.next!=p)
            front = front.next;
        return front;
    }
    public ListNode<T> getLast()                     //返回单链表最后一个元素结点，O(n)
    {
        ListNode<T> p=this.head.next;
...
/*    public SinglyLinkedList(T[] element)
    ...
    public SinglyLinkedList<T> sub(int i, int n)
    {
        ...
        ListNode<T> rear=list.head;                            //复制子表到list链表
        for (int j=0;  p!=null && j<n;  j++, p=p.next)
        {
            rear.next = new ListNode<T>(p.val,null);          //在rear结点之后插入值为p.val的结点
            rear = rear.next;
        }
        return list;                                       //返回对象引用
    }
    ...
    {
        for (ListNode<T> q=list.head.next;  q!=null;  q=q.next)
        {
            ListNode<T> p=head.next;
            while (p!=null && !p.val.equals(q.val))      //比较q结点值是否包含在当前单链表中
                p = p.next;
            if (p==null)                                   //不包含
                return false;
        }
    ...
    public void insert(ListNode<T> front, SinglyLinkedList<T> list)
    {
        for (ListNode<T> q=list.head.next;  q!=null;  q=q.next)
        {
            front.next = new ListNode<T>(q.val, front.next);  //在p结点之后插入值为q.val的结点
            front = front.next;
        }
    }
...
/*或者
    ...
    public ListNode<T> index(SinglyLinkedList<T> pattern)
    {
        return index(head.next, pattern);
    }
    //返回this单链表从start结点开始首次出现的与pattern匹配的子表首结点。
    //BF模式匹配。逐个结点比较，一重循环。
    public ListNode<T> index(ListNode<T> start, SinglyLinkedList<T> pattern)
    {
        if (pattern.isEmpty())                             //若无此句，则返回start，错误
            return null;
        ListNode<T> p=start, q=pattern.head.next;
        while (p!=null && q!=null)
        {
            if (p.val.equals(q.val))                     //继续比较下一个结点
            {   p=p.next;
                q=q.next;
            }
            else                                           //一次匹配失败
    ...
    public Node<T> index(Node<T> start, SinglyLinkedList<T> pattern)
    {
        ...
        while (start!=null)
        {
            ...
            while (p!=null && q!=null && p.val.equals(q.val))//继续比较下一个结点
            {
                p=p.next;
                q=q.next;
            }
    ...
    public void removeAll(SinglyLinkedList<T> pattern)
    {
        ...
        if (pattern.isEmpty())                             //若无此句，则死循环，错误
            return;
        ListNode<T> start=this.head.next, front=this.head;
        while (start!=null)
        {
            ListNode<T> p=start, q=pattern.head.next;
            while (p!=null && q!=null && p.val.equals(q.val)) //一次匹配
            {
                p=p.next;
                q=q.next;
            }
    ...
    public void replaceAll(SinglyLinkedList<T> pattern, SinglyLinkedList<T> dest)
    {
        ...
        if (pattern.isEmpty())                             //若无此句，则将dest插入到start结点之后，错误
            return;
        ListNode<T> start=this.head.next, front=this.head;     //front是start的前驱结点
        while (start!=null)                                //start指向每次匹配的起始结点
        {
            ListNode<T> p=start, q=pattern.head.next;
            while (p!=null && q!=null && p.val.equals(q.val))//一次匹配的多次比较
            {   p=p.next;
                q=q.next;
            }
            if (q!=null)                                   //匹配失败，进行下次匹配
            ...
            {   front.next = p;
                start=p;
                ListNode<T> d=dest.head.next;
                while (d!=null)                            //将dest剩余结点深拷贝插入到this单链表front之后p之前
                {
                    front.next = new ListNode<T>(d.val, p); 
                    front = front.next;
                    d = d.next;
                }
            }
...
/*    public String toString()                               //返回单链表描述字符串
    ...
    {
        return p==null && q==null || 
               p!=null && q!=null && p.val.equals(q.val) && equals(p.next, q.next);
    }
    */
    ...
    public void remove(T x)                   //删除首次出现的值为x的结点，若没找到指定结点则不删除。O(n)
    {
        if (x==null)
            return;
        ListNode<T> front=this.head, p=front.next;
    ...
    {
        ListNode<T> current=SinglyLinkedList.this.head;        //当前结点，初值为外部类单链表头结点
        ListNode<T> front=null;                                //当前结点的前驱结点
