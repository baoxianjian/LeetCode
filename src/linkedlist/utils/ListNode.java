...
public class ListNode<T>                             //单链表结点类，T指定结点的元素类型
{
    public T val;                               //数据域，保存数据元素
    public ListNode<T> next;                         //地址域，引用后继结点

    public ListNode(T val, ListNode<T> next)            //构造结点，val指定数据元素，next指定后继结点
    {
        this.val = val;
        this.next = next;
    }
    public ListNode(T val)            //构造结点，val指定数据元素
    {
        this.val = val;
    }
    public ListNode()
